...
dtype_y = 'uint8'
model_fn = 'amazon_5x5resizelarger.pkl'

img_size = 128
max_offset = 10
window = (img_size-2*max_offset, img_size-2*max_offset)

train_x, train_y, train_names = load_npys(which_set='train', image_size=img_size, resizing_method='bluntresize', seed=6167)
valid_x, valid_y, valid_names = load_npys(which_set='valid', image_size=img_size, resizing_method='bluntresize', seed=6167)
...
if os.path.isfile(model_fn) and True:
    ...
    mlp = MLP([
        ...
        # 52 -> 24
        GaussianDropout(0.03),
        ConvolutionalLayer((5, 5), 48, leaky_relu_alpha=1),
        MaxPool((2, 2)),
        NonLinearity(),

        # 24 -> 12
        GaussianDropout(0.03),
        ConvolutionalLayer((5, 5), 80, pad=2, leaky_relu_alpha=prelu_alpha),
        MaxPool((2, 2)),
        NonLinearity(),

        # 12 -> 6
        GaussianDropout(0.03),
        ConvolutionalLayer((5, 5), 160, pad=2, leaky_relu_alpha=prelu_alpha),
        MaxPool((2, 2)),
        NonLinearity(),

        # 6 -> 3
        GaussianDropout(0.03),
        ConvolutionalLayer((5, 5), 304, pad=2, leaky_relu_alpha=prelu_alpha),
        MaxPool((2, 2)),
        NonLinearity(),
