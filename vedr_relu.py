...
seed = 55678
np.random.seed(seed)
model_fn = 'vedr_relu.pkl'

img_size = 165
max_offset = 10
window = (img_size-2*max_offset, img_size-2*max_offset)

train_x, train_y, train_names = load_npys(which_set='train', image_size=img_size, resizing_method='shrink', seed=seed)
valid_x, valid_y, valid_names = load_npys(which_set='valid', image_size=img_size, resizing_method='shrink', seed=seed)
